...
- Embeddings are centralized at `POST /v1/embeddings` and `POST /api/v1/embeddings`; Router authenticates the request, rewrites the model to the selected upstream ID, and proxies to `AKURAI_ROUTER_EMBEDDINGS_URL`. Default selected model is Titan's `embeddinggemma` served over the AkurAI VPN mesh at `http://100.88.0.2:8081/v1/embeddings`.
